...
{
	if (MessageBoxW(NULL, L"This is a GDI Only, Run?", L"2,3,7,8-Tetrachlorodibenzodioxin2.0-safety.exe by pankoza and MalwareRenders (huge thx Coder-Linjan for some codes)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? It contains flashing lights - NOT for epilepsy", L"2,3,7,8-Tetrachlorodibenzodioxin2.0.exe-safety - Last Warning", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
