...
{
	if (MessageBoxW(NULL, L"Run GDI Only? This will not delete windows", L"flv.exe by pankoza (GDI Only)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Clicking yes will initiate the GDI Only and it contains flashing lights - NOT for epilepsy", L"flv.exe - Last Warning (GDI Only)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
